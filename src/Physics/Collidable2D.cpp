...
namespace tewi
{
    namespace Physics
    {
        ...
        bool checkAABB(const std::vector<Collidable2D>& firstGroup,
                       const std::vector<Collidable2D>& secondGroup)
        {
            ...
            for (const auto& first : firstGroup)
            {
                ...
                {
                    if (!((first.refPos->x <
                           second.refPos->x + second.refSize->x) &
                          (first.refPos->x + first.refSize->x >
                           second.refPos->x) &
                          (first.refPos->y <
                           second.refPos->y + second.refSize->y) &
                          (first.refPos->y + first.refSize->y >
                           second.refPos->y)))
                    {
                        test = false;
                    }
        ...
        {
            float distance_x = (first.refPos->x + first.radius) -
                               (second.refPos->x + second.radius);
            float distance_y = (first.refPos->y + first.radius) -
                               (second.refPos->y + second.radius);

            float distance =
                (distance_x * distance_x + distance_y * distance_y);

            return distance < (first.radius * first.radius +
                               second.radius * second.radius);
        }

        bool checkRadius(const std::vector<Collidable2D>& firstGroup,
                         const std::vector<Collidable2D>& secondGroup)
        {
            bool test = true;
            for (const auto& first : firstGroup)
            {
                for (const auto& second : secondGroup)
                {
                    ...
                    if (!(distance < (first.radius * first.radius +
                                      second.radius * second.radius)))
                    {
                        test = false;
                    }
